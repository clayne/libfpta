...
TEST(Nullable, SchemaReloadAfterAbort) {
  /*
   *  1. В "коммандере" создаем БД с одной таблицей, в которой несколько
   *     неиндексируемых не-nullable колонок.
   *
   *  2. В "корреляторе" пробуем вставить некорректную запись без значения
   *     обязательной (не-nullable) колонки, затем прерывает эту транзацию.
   *
   *  3. В "коммандере" удаляем и заново создаем таблицу, с тем-же составом
   *     колонок, но теперь описываем их как необазятельные (nullable).
   *
   *  4. В "корреляторе" снова делаем вставку строки без значений нескольких
   *     колонок, но прерываем транзакцию.
   *
...
TEST(Nullable, SchemaReload) {
  /*
   *  1. В "коммандере" создаем БД с одной таблицей, в которой несколько
   *     индексируемых не-nullable колонок без контроля уникальности значений.
   *
   *  2. В "корреляторе" пробуем вставить некорректную запись без значения
   *     обязательной (не-nullable) колонки, затем прерывает эту транзацию.
   *
   ...
   *
   *  4. В "корреляторе" снова делаем вставку строки без значений нескольких
   *     колонок и фиксируем транзакцию.
   *
   *  5. Завершаем операции и освобождаем ресурсы.
   */
  const bool skipped = GTEST_IS_EXECUTION_TIMEOUT();
